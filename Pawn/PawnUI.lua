...
function PawnUIFrame_StatsList_SelectStat(Index)
	...
	if Index > 0 and ThisScale then
		...
		if ThisStatIsIgnored or WeaponSet == nil then
...
function PawnUIOptionsTabPage_OnShow()
	...
	PawnUIFrame_ShowUpgradeAdvisorsCheck:SetChecked(PawnCommon.ShowUpgradeAdvisors)
...
function PawnUIFrame_ShowUpgradeAdvisorsCheck_OnClick()
	PawnCommon.ShowUpgradeAdvisors = PawnUIFrame_ShowUpgradeAdvisorsCheck:GetChecked() ~= nil
	PawnResetTooltips()
end
...
function PawnUI_LootUpgradeAdvisor_OnShow(self)
	if not PawnCommon.ShowUpgradeAdvisors then self:Hide() return end
	...
	if UpgradeInfo then
		...
		if ShowScaleNames then
			...
			for _, ThisUpgradeData in pairs(UpgradeInfo) do
				...
				local Annotation = PawnLocal.TooltipUpgradeAnnotation
				if ThisUpgradeData.PercentUpgrade >= 100 then Annotation = PawnLocal.TooltipBigUpgradeAnnotation end
				local SetAnnotation = ""
				...
				local ThisText = format(Annotation, format("|n%s%s:", PawnGetScaleColor(ScaleName), ThisUpgradeData.LocalizedScaleName), ThisUpgradeData.PercentUpgrade * 100, SetAnnotation)
...
function PawnUI_OnQuestInfo_ShowRewards()
	...
	if not PawnCommon.ShowUpgradeAdvisors then return end
