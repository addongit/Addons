...
-- 
-- Version 1.5.6: restored the current values option
------------------------------------------------------------
...
function PawnInitializeOptions()
	...
	if (not PawnCommon.LastVersion) or (PawnCommon.LastVersion < 1.506) then
		-- When upgrading to 1.5.6, invalidate all best item data.
		PawnInvalidateBestItems()
	end
				...
				if BestData then
					...
					if BestValue then
						...
						if Item.ID == BestData[2] then
							...
							break
						elseif ThisValue > BestValue * 1.0005 and (IsScalingHeirloom or PawnPlayerIsAtMaxLevel() or UnitLevel("player") > BestMaxHeirloomLevel) then
							...
							if Item.ID == BestData[5] and (IsScalingHeirloom or PawnPlayerIsAtMaxLevel() or UnitLevel("player") > BestData[6]) then
