...
-- 
-- Version 1.5.4: incomplete item data fix for quest upgrade advisor; reforge double-dip fix
------------------------------------------------------------
...
function PawnOnLogout()
	...
	if (not PawnCommon.ShowUpgradesOnTooltips) and (not PawnCommon.ShowUpgradeAdvisors) then
...
function PawnInitializeOptions()
	...
	if PawnCommon.ShowUpgradeAdvisors == nil then PawnCommon.ShowUpgradeAdvisors = true end
	...
	for ScaleName, Scale in pairs(PawnCommon.Scales) do
		...
		if ShowScale then -- Skip all disabled scales.  PawnGetSingleValueFromItem will calculate them on-demand if necessary.
			...
			if UnenchantedItem then
				UnenchantedValue, UseRed, UseYellow, UseBlue = PawnGetItemValue(UnenchantedItem, ItemLevel, UnenchantedItemSocketBonus, ScaleName, ShowScale and DebugMessages and PawnCommon.ShowUnenchanted, NoNormalization, false)
			end
			if Item then
...
function PawnIsItemAnUpgrade(Item, DoNotRescan)
	...
	for ScaleName, Scale in pairs(PawnCommon.Scales) do
		...
		if PawnIsScaleVisible(ScaleName) and not
			...
			(Scale.DoNotShow2HUpgrades and InvType == "INVTYPE_2HWEAPON")
		then
...
function PawnFindOptimalReforgingCore(ScaleName, Scale, Values, Stats, NoInstructions)
	...
	if Scale.NormalizationFactor and Scale.NormalizationFactor > 0 then
