...
-- 
-- Version 1.5.5: prismatic and cogwheel sockets, armor specializations, new options
------------------------------------------------------------
...
-- Used to be options... keeping them here for now just in case I change my mind.
local PawnCommon_ShowEnchanted = false
local PawnCommon_ShowUnenchanted = true
	...
	for ScaleName, Scale in pairs(PawnCommon.Scales) do
		...
		if ShowScale then -- Skip all disabled scales.  PawnGetSingleValueFromItem will calculate them on-demand if necessary.
			...
			if UnenchantedItem then
				UnenchantedValue, UseRed, UseYellow, UseBlue = PawnGetItemValue(UnenchantedItem, ItemLevel, UnenchantedItemSocketBonus, ScaleName, ShowScale and DebugMessages and PawnCommon_ShowUnenchanted, NoNormalization, NoNormalization)
			end
			if Item then
				if ShowScale and DebugMessages and PawnCommon_ShowEnchanted and PawnCommon_ShowUnenchanted then
					PawnDebugMessage(" ")
					PawnDebugMessage(PawnLocal.EnchantedStatsHeader)
				end
				Value = PawnGetItemValue(Item, ItemLevel, SocketBonus, ScaleName, ShowScale and DebugMessages and PawnCommon_ShowEnchanted, NoNormalization, true)
			end
	...
	for _, Entry in pairs(ItemValues) do
		...
		if PawnIsScaleVisible(ScaleName) then
			...
			if OnlyFirstValue then
				...
				if not PawnCommon_ShowEnchanted then Value = 0 end
				if not PawnCommon_ShowUnenchanted then UnenchantedValue = 0 end
			end
...
function PawnFindBestGems(ScaleName, FindRed, FindYellow, FindBlue, FindMeta, FindCogwheel)
	...
	for _, GemData in pairs(GemTable) do
		if FindMeta or FindCogwheel or (FindRed and GemData[2]) or (FindYellow and GemData[3]) or (FindBlue and GemData[4]) then
			...
			if ThisGem then
				local ThisValue = PawnGetItemValue(ThisGem.UnenchantedStats, ThisGem.Level, nil, ScaleName, true, true, true)
...
function PawnIsItemAnUpgrade(Item, DoNotRescan)
	...
	for ScaleName, Scale in pairs(PawnCommon.Scales) do
		...
		then
			...
			while InvType do
				...
				if BestData then
					local BestValue = BestData[3] or BestData[1]
					local BestItem = BestData[4] or BestData[2]
					if BestValue then
						...
						if Item.ID == BestData[2] then
							...
							break
						elseif ThisValue > BestValue * 1.0005 then
							-- Hooray, it's an upgrade!  Add it to the table.
							-- (Only count upgrades that are at least 0.05% better.)
	...
	--	end
	--else
	--	VgerCore.Message("That item is not an upgrade.")
	--end
...
function PawnFindBestItems(ScaleName, InventoryOnly)
	...
	-- = { ["INVTYPE_HEAD"] = { 123.45, itemID }, ...,  ["INVTYPE_FINGER"] = { 67.8, itemID, 50.50, second itemID }, ... }
	...
	local function CheckItem(ScaleName, BestItems, Item, PreviousItemID)
		...
		if BestOfType == nil or Value > BestOfType[1] then
			...
			if InvType == "INVTYPE_FINGER" or InvType == "INVTYPE_WEAPON" then
				...
				BestOfType[3] = BestOfType[1]
				BestOfType[4] = BestOfType[2]
			end
			BestOfType[1] = Value
		...
		elseif
			...
			(BestOfType[3] == nil or Value > BestOfType[3]) and
...
function PawnAssignOneHandedBestItems(Scale, BestItems)
	...
	if BestItems.INVTYPE_WEAPON then
		...
		if BestItems.INVTYPE_WEAPONMAINHAND and BestItems.INVTYPE_WEAPONMAINHAND[2] == BestItems.INVTYPE_WEAPON[2] then
			BestItems.INVTYPE_WEAPON[1] = BestItems.INVTYPE_WEAPON[3]
			BestItems.INVTYPE_WEAPON[2] = BestItems.INVTYPE_WEAPON[4]
			BestItems.INVTYPE_WEAPON[3] = nil
			BestItems.INVTYPE_WEAPON[4] = nil
		elseif BestItems.INVTYPE_WEAPONMAINHAND and BestItems.INVTYPE_WEAPONMAINHAND[2] == BestItems.INVTYPE_WEAPON[4] then
			BestItems.INVTYPE_WEAPON[3] = nil
			BestItems.INVTYPE_WEAPON[4] = nil
		end
		if BestItems.INVTYPE_WEAPONOFFHAND and BestItems.INVTYPE_WEAPONOFFHAND[2] == BestItems.INVTYPE_WEAPON[2] then
			...
			BestItems.INVTYPE_WEAPON[3] = nil
			BestItems.INVTYPE_WEAPON[4] = nil
		elseif BestItems.INVTYPE_WEAPONOFFHAND and BestItems.INVTYPE_WEAPONOFFHAND[2] == BestItems.INVTYPE_WEAPON[4] then
			BestItems.INVTYPE_WEAPON[3] = nil
			BestItems.INVTYPE_WEAPON[4] = nil
		end
		
		Best1H = BestItems.INVTYPE_WEAPON[1] or 0
		SecondBest1H = BestItems.INVTYPE_WEAPON[3] or 0
	end
	...
	then
		...
		if BestItems.INVTYPE_WEAPON and BestItems.INVTYPE_WEAPON[1] then
			BestItems.INVTYPE_WEAPONMAINHAND = { BestItems.INVTYPE_WEAPON[1], BestItems.INVTYPE_WEAPON[2] }
		else
			BestItems.INVTYPE_WEAPONMAINHAND = nil
		end
		if BestItems.INVTYPE_WEAPON and BestItems.INVTYPE_WEAPON[3] then
			BestItems.INVTYPE_WEAPONOFFHAND = { BestItems.INVTYPE_WEAPON[3], BestItems.INVTYPE_WEAPON[4] }
		else
	...
	then
		...
		if BestItems.INVTYPE_WEAPON and BestItems.INVTYPE_WEAPON[1] then
			BestItems.INVTYPE_WEAPONOFFHAND = { BestItems.INVTYPE_WEAPON[1], BestItems.INVTYPE_WEAPON[2] }
		else
			BestItems.INVTYPE_WEAPONOFFHAND = nil
		end
	else
	...
	for ScaleName, Scale in pairs(PawnCommon.Scales) do
		...
		if CharacterOptions then
			...
			if BestItems then
				...
				if ItemsForSlot and (ItemsForSlot[2] == ItemID or ItemsForSlot[4] == ItemID) then
					RemovedItem = true
...
function PawnPlayerUsesCogwheels()
	...
	return UnitLevel("player") >= 81 and (GetProfessionInfo(Profession1) == PawnLocal.EngineeringName or GetProfessionInfo(Profession2) == PawnLocal.EngineeringName)
end
