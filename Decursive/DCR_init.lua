...
function D:VersionWarnings()
    ...
    if (("2.6.1_beta_3"):lower()):find("beta") or ("2.6.1_beta_3"):find("RC") or ("2.6.1_beta_3"):find("Candidate") or alpha then
        ...
        if D.VersionTimeStamp ~= 0 then
            ...
            if time() > D.VersionTimeStamp + VersionLifeTime then
                ...
                if time() - self.db.global.LastExpirationAlert > 48 * 3600  then
                    StaticPopup_Show ("Decursive_Notice_Frame", "|cff00ff00Decursive version: 2.6.1_beta_3|r\n\n" .. "|cFFFFAA66" .. L["DEV_VERSION_EXPIRED"] .. "|r");
        ...
        if self.db.global.NonRealease ~= "2.6.1_beta_3" then
            self.db.global.NonRealease = "2.6.1_beta_3";
...
T._LoadedFiles["DCR_init.lua"] = "2.6.1_beta_3";
...
@project-revision@
    ...
    Note: does not work for git
d226a3d608d8aaa848fc6e1c4a9c0498f813cdbe
