---------------------------------------------------------------------------------------------
QHLVERSION = "QuestHelperLite v4.0.3.1"
---------------------------------------------------------------------------------------------
-- Author
--
...
function QuestHelperLite:GetWorld(map, floor)
	...
	if systemParent then return systemParent;
	else return -1; end
end

function QuestHelperLite:GetWorldPosition(map, floor, x, y)

	local world, world_floor = QHL:GetWorld(map, floor), 0;
	...
	if (not world_x or not world_y) or (world_x <= 0 or world_y <= 0) or (world_x >= 1 or world_y >=1) then
		world_floor = 1;
		world_x, world_y = QHL.Astrolabe:TranslateWorldMapPosition(map, floor, x, y, world, world_floor);
	end
