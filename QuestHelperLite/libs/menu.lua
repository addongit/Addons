...
--
QuestHelperLite.active_menu = nil
...
function QuestHelperLite:CreateFrame(parent)
	...
	if free_frames > 0 then
		...
		frame = CreateFrame("Button", QHL:Concat("QHL_RecycledFrame", frame_count), parent);
  	end
...
function QuestHelperLite:RecycleFrame(frame)
...
function QuestHelperLite:CreateTable()
...
function QuestHelperLite:RecycleTable(table)
...
function QuestHelperLite:CreateText(parent, font, msg)
...
function QuestHelperLite:RecycleText(text)
...
function QuestHelperLite:CreateTexture(parent, r, g, b, a)
...
function QuestHelperLite:RecycleTexture(texture)
...
function QuestHelperLite:CreateIconTexture(parent, id)

  local icon = QHL:CreateTexture(parent, "Interface\\AddOns\\QuestHelperLite\\Art\\Icons.tga")
...
local function Menu_ShowAtCursor(self, auto_release)
  ...
  if not self.close_item then
    self.close_item = QHL:CreateMenuItem(self, QHL_LOCALE_MENU_CLOSE)
  ...
  local parent = not UIParent:IsVisible() and QHL.map_overlay or UIParent
  ...
  if QHL.active_menu and QHL.active_menu ~= self then
        QHL.active_menu:DoHide()
  end

  QHL.active_menu = self
end

function QuestHelperLite:CreateMenu()
...
function QuestHelperLite:ReleaseMenu(menu)
...
local function MenuItem_SetSubmenu(self, menu)
  ...
  menu.parent = self
  self.submenu = menu
  self:AddTexture(QHL:CreateIconTexture(self, 9))
end
...
function QuestHelperLite:CreateMenuItem(menu, text)
  ...
  item.text = self:CreateText(item, QHL.font.sans, text);
...
function QuestHelperLite:ReleaseMenuItem(item)
  item:Hide()
...
function QuestHelperLite:CreateMenuTitle(menu, title)
  local item = self:CreateMenuItem(menu, title)                 
  
  local f1 = self:CreateTexture(item, "Interface\\AddOns\\QuestHelperLite\\Art\\Fluff.tga")
  local f2 = self:CreateTexture(item, "Interface\\AddOns\\QuestHelperLite\\Art\\Fluff.tga")
  ...
  item.text:SetFont(self.font.fancy, 11)
end
