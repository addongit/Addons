...
if select(2, UnitClass("player")) == "DEATHKNIGHT" then	
	...
	function CLCDK:CreateUI()
		...
		local function CreateRuneBar()
			...
			frame:SetStatusBarColor(1, 0.2, 0.2, 0.6)	
			frame:Raise()
			frame:GetStatusBarTexture():SetBlendMode("DISABLE")
			frame:SetBackdrop{bgFile = 'Interface\\Tooltips\\UI-Tooltip-Background', tile = false, insets = {left = 0, right = 0, top = 0, bottom = 0},}
		...
		if CLCDK_Settings.Rune or CLCDK_Settings.RuneBars then
			...
			local function runetext(i)
				...
				if CLCDK_Settings.RuneBars then
					...
					CLCDK.RuneBars[place]:SetBackdropColor(r, g, b)
	...
	CLCDK:SetScript("OnEvent", function(_, e, ...)
		if loaded then 
			...
			if e == "COMBAT_LOG_EVENT_UNFILTERED" then		
				local _, event, _, _, casterName, _, targetguid, targetName, _, _, spellName = ...			
