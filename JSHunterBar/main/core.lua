...
JSHB.anchorFrame.indicatorBar	= CreateFrame("Frame", "JSHB_MainFrame_Anchor_ccIcons", UIParent)
...
function JSHB.makeTukui(frame, isNotIcon)
...
local function setCCTimers()
	...
	for i=1,4 do -- Allocating 4 frames, more than enough, low overhead
		...
		if DB.enabletukui then
			JSHB.cCIconFrames[i].border = JSHB.makeTukui(JSHB.cCIconFrames[i])
		end
...
local function refreshDebuffAlert(spellID, expireTime, stacks, whatEvent)
...
local function setDebuffAlert()
	...
	for i=1,6 do -- Allocating 6 frames, more than enough, low overhead
		...
		if DB.enabletukui then
			JSHB.debuffIconFrames[i].border = JSHB.makeTukui(JSHB.debuffIconFrames[i])
		end
...
local function setMarkReminder()
	...
	if DB.enabletukui then
		JSHB.markFrame.border = JSHB.makeTukui(JSHB.markFrame)
	end
...
--	JSHB.tranqFrame:SetAttribute("unit", "target")
	...
	if DB.enabletukui then
		JSHB.tranqFrame.border = JSHB.makeTukui(JSHB.tranqFrame)
	end
	...
	for x=1,#DB.timers[JSHB.specs[JSHB.currentTree]] do
		...
		if (DB.timers[JSHB.specs[JSHB.currentTree]][x][4] ~= JSHB.pos.LEFT) and (DB.timers[JSHB.specs[JSHB.currentTree]][x][4] ~= JSHB.pos.RIGHT) then
			...
			if DB.enabletukui and (DB.timers[JSHB.specs[JSHB.currentTree]][x][4] ~= JSHB.pos.CENTER) then
				JSHB.timerFrames[i].border = JSHB.makeTukui(JSHB.timerFrames[i])
			end
			...
			if DB.enabletukui then
				JSHB.iconTimerFrames[j].border = JSHB.makeTukui(JSHB.iconTimerFrames[j])
			end
...
local function setTranqables()
	...
	for i=1,40 do
		...
		if DB.enabletukui then
			JSHB.tranqablesFrames[i].border = JSHB.makeTukui(JSHB.tranqablesFrames[i])
		end
...
function mF:ApplyLock()
	...
	if JSHB.barLocked then -- LOCK
		...
		enableFrameLock(JSHB.anchorFrame.indicatorBar, "indicatorBarAnchorPoint", "indicatorBarAnchorPointRelative", "indicatorBarAnchorPointOffsetX", "indicatorBarAnchorPointOffsetY", JSHB.setupClassIndicators)
		...
		if DB.enableindicator then
			enableFrameUnlock(JSHB.indicatorFrame, JSHB.anchorFrame.indicatorBar, L.moveindicator, JSHB.indicatorFrame:GetWidth(), JSHB.indicatorFrame:GetHeight(),
				"indicatorBarAnchorPoint", "indicatorBarAnchorPointRelative", "indicatorBarAnchorPointOffsetX", "indicatorBarAnchorPointOffsetY")
		end
	end
end
...
function mF:ApplyOptions()
	...
	if DB.enabletukui then
		mF.border = JSHB.makeTukui(mF, true)
...
mF:SetScript("OnEvent", function(self, event, ...)
	...
	if event == "ADDON_LOADED" then
		...
		for x,y in pairs(JSHB.defaults["indicator"]) do
			if JSHB_SavedOptions[x] == nil then JSHB_SavedOptions[x] = y end
		end
	...
	elseif event == "PLAYER_LOGIN" then
		...
		mF:RegisterEvent("VARIABLES_LOADED")
	...
	elseif (event == "COMBAT_LOG_EVENT_UNFILTERED") then
		...
		if DB.enablecctimers == true then
			...
			if (select(2, ...) == "SPELL_AURA_APPLIED") and (select(4, select(1, ...)) == UnitGUID("player")) then

				if select(10, ...) == 3355 then -- Freezing trap Aura

					i = (bit.band(select(9, select(1, ...)), COMBATLOG_OBJECT_CONTROL_PLAYER) > 0) and 8 or (60 + (6 * select(5, GetTalentInfo(3, 5))))

					addCCTimer(1499, select(7, select(1, ...)), i) -- account for trap mastery

				elseif select(10, ...) == 19386 then -- wyvern

					addCCTimer(19386, select(7, select(1, ...)), 30) -- wyvern
				end

			elseif (select(2, ...) == "SPELL_AURA_REFRESH") and (select(4, select(1, ...)) == UnitGUID("player")) then

				if select(10, ...) == 3355 then -- Freezing Trap Aura

					i = (bit.band(select(9, select(1, ...)), COMBATLOG_OBJECT_CONTROL_PLAYER) > 0) and 8 or (60 + (6 * select(5, GetTalentInfo(3, 5))))
					refreshCCTimer(1499, select(7, select(1, ...)), i) -- account for trap mastery

				end
			elseif (select(2, ...) == "SPELL_AURA_REMOVED") and (select(4, select(1, ...)) == UnitGUID("player")) then

				if select(10, ...) == 3355 then -- Freezing Trap Aura

					stopCCTimer(1499, select(7, select(1, ...)))

				elseif select(10, ...) == 19386 then -- wyvern

					stopCCTimer(19386, select(7, select(1, ...))) -- wyvern
				end
			end
		end
		...
		if DB.enabletranqannounce == true then
			...
			if (select(2, ...) == "SPELL_DISPEL") and (select(4, select(1, ...)) == UnitGUID("player")) and (select(7, select(1, ...)) ~= UnitGUID("pet")) then

				SendChatMessage(L.tranqremoved .. "|cff71d5ff|Hspell:" .. select(2, select(12, ...)) .. "|h[" .. select(2, select(13, ...)) .. "]|h|r"
						.. L.tranqfrom..select(8, select(1, ...))..".", getChatChan(DB.tranqannouncechannel), nil, GetUnitName("player"))
			end
		end
		
		if (select(2, ...) == "SPELL_CAST_SUCCESS") and (select(10, ...) == 34477) and (select(4, select(1, ...)) == UnitGUID("player")) then
			JSHB.misdirectionToPlayer = select(8, ...)
		end
		...
		if (DB.enablemdcastannounce == true) then
			...
			elseif (select(2, ...) == "SPELL_CAST_FAILED") and (select(10, ...) == 34477) and (select(4, select(1, ...)) == UnitGUID("player")) then
				...
				if UnitIsPlayer(JSHB.mDHoverTarget) and (UnitInParty(JSHB.mDHoverTarget) or UnitInRaid(JSHB.mDHoverTarget)) then
					...
					if (select(13, ...) == SPELL_FAILED_NOT_ON_MOUNTED) or (select(13, ...) == SPELL_FAILED_NOT_ON_SHAPESHIFT) then
		...
		if DB.enablemdtargetwhisper == true then -- aggro transfer (35079)
			...
			if (select(2, ...) == "SPELL_AURA_APPLIED") and (select(10, ...) == 35079) and (select(4, select(1, ...)) == UnitGUID("player")) then
			...
			if (select(2, ...) == "SPELL_AURA_REMOVED") and (select(10, ...) == 35079) and (select(4, select(1, ...)) == UnitGUID("player")) then
		...
		if DB.enablemdoverannounce == true then
			...
			if (select(2, ...) == "SPELL_AURA_REMOVED") and (select(10, ...) == 34477) and (select(4, select(1, ...)) == UnitGUID("player")) then
		...
		if DB.enabledebuffalert then
			...
			local v = select(10, ...)
			if (select(2, ...) == "SPELL_AURA_APPLIED") and (select(7, select(1, ...)) == UnitGUID("player")) then
			...
			elseif ((select(2, ...) == "SPELL_AURA_REFRESH") or (select(2, ...) == "SPELL_AURA_APPLIED_DOSE")) and (select(7, select(1, ...)) == UnitGUID("player")) then
				...
				if (tContains(JSHB.debuffalerts, v) ~= nil) or (tContains(DBG.customaura, v) ~= nil) then
					...
					if select(11, UnitAura("player", GetSpellInfo(v), nil, "HARMFUL")) == v then
						...
						refreshDebuffAlert(v, select(6, UnitAura("player", GetSpellInfo(v), nil, "HARMFUL")),
							select(4, UnitAura("player", GetSpellInfo(v), nil, "HARMFUL")), select(2, ...))
					else
						refreshDebuffAlert(v, select(6, UnitAura("player", GetSpellInfo(v), nil, "HELPFUL")),
							select(4, UnitAura("player", GetSpellInfo(v), nil, "HELPFUL")), select(2, ...))
					end
				end

			elseif (select(2, ...) == "SPELL_AURA_REMOVED") and (select(7, select(1, ...)) == UnitGUID("player")) then
