...
-- 	Define the options panel
	...
	function LeaPlusOptionsPanel:OptionsPanel()
		...
		OptionsText("Error Frame", 200, -232);
		LeaPlusCheckbox("LeaPlusErrorMsg", 		"Hide error messages",		 	200, -252, 	"If checked, error messages (eg. 'Not enough rage') will not be shown in the error frame.  'Inventory is full' and 'Quest log is full' will be shown regardless of this setting.")
		LeaPlusCheckbox("LeaPlusInfoMsg", 		"Hide info messages", 			200, -272, 	"If checked, information and system messages (eg. quest updates) will not be shown in the error frame.")

		OptionsText("Text Size", 200, -312);
		LeaPlusCheckbox("LeaPlusQuestFont", 	"Resize quest log text", 		200, -332, 	"If checked, quest log text will be resized according to the value set by the slider.  This will also affect a few other windows.")
		LeaSlider("LeaPlusScaleSlider","LeaPlusQuestSize","",10, 36, 1, 200, -362,"%.0f", "10", "36")
		...
		LeaPlusTimer.tiptext = "Shows your random dungeon cooldown if the rewards window is showing.\n\nThis button has no effect if the rewards window is not showing."
