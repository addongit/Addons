...
function XBar_StdTexture(mod,texture,spellname)
	...
	-- Will highlight the buffs the player has
	for i = 1, 50 do
...
function XBarCore.ClickHandler(self,button,down)
	local bn=self:GetName();
	local pfn=self:GetParent():GetName();
	local v,d,i,t,c,_;
	...
	if (XBarCore.FrameDB[pfn].IgnoreClicks) then
		-- self is a PostClick event, so we have to unset self ourselves after a move instead of in the OnMouseUp event
	...
	if (a) then
		...
		elseif v.Type==XBAR_TTRACKING then
			_,_,t=GetTrackingInfo(i); -- Get the active state
			if (t) then
				t=nil;
			else
				t=1;
...
function XBarCore.CountSpells(mod)
	...
	-- self function performs the common task of enumerating the number of
	...
	for i,v in pairs(XBarCore.ModData[mod].ActionList) do
		...
		elseif v.Type==XBAR_THEADER then
			...
			-- Whatever self is, we want to include it in the list
			rchecks=rchecks+1;
			rbuttons=rbuttons+1;
		end
	end
	...
	if (p1) then
		...
		if (p1) then
			...
			if (p1) then
				...
				if (s) then
					-- Lets strip out self line ahead of time so we can ignore any following lines
					p2=strfind(s,strchar(10),1,true);
					if (p2) then
						s=strsub(s,1,p2-1);
					end
...
--[[function XBarCore.FixMenu(level)
	-- self was added since the dropdown frame menus appear to spontaneously change frame level to behind the backdrop.
	local i,fs1,fs2;
	...
	for i=1,UIDROPDOWNMENU_MAXBUTTONS do
		fs1 = _G["DropDownList"..tostring(level).."Backdrop"]:GetFrameLevel();
		fs2 = _G["DropDownList"..tostring(level).."Button"..tostring(i)]:GetFrameLevel();
		if fs2 < fs1 then
			_G["DropDownList"..tostring(level).."Button"..tostring(i)]:SetFrameLevel(fs1+1);
		end
	end
end
]]--
...
function XBarCore.GenerateDB(mod)
	-- self function generates the rest of the data in the internal DB that a plugin did not/cannot provide.
	local a,i,d,v,sn,tex,x,r,s,si;
	
	-- self only updates the default sort order, the actual sort order will be saved elsewhere
	...
	for a,v in pairs(XBarCore.ModData[mod].ActionList) do
		for i,d in pairs(v.Data) do
			...
			elseif v.Type == XBAR_TTRACKING then
				-- This is handled down below
			elseif v.Type == XBAR_TITEM and type(d.ID) == "number" then
				-- The ID for items might already exist
			else
				d.ID=1;
			end
			if i==1 then -- All information about this button will be retrieved from the first action
				...
				if v.Type == XBAR_TSPELL or v.Type == XBAR_TSPELLHAX then
					if d.ID then
						...
						if (not tex) and XBarCore.ModData[mod].ActionList[a].Texture then
							-- If we unlearn a spell while in combat, check to see if we saved it (self only happens for pets that die and the body disappears).
				...
				elseif v.Type == XBAR_TITEM then
					-- Requires the ID to be passed now since WoW patch 4.0.1
					if type(d.ID) ~= "number" then
						-- if it is not a number, need to convert it
						d.ID = tonumber(d.Data);
						if type(d.ID) == "number" then
							d.Data = tostring(GetItemInfo(d.ID));
						end
					else
						-- It already is a number
						d.Data = GetItemInfo(d.ID);
						if d.Data == nil then
							d.Data = "Unknown: "..tostring(d.ID);
						else
							d.Data = tostring(d.Data);
						end
					end
					if d.ID == nil or d.ID == 0 or d.Data == nil or d.Data == "" then
						-- Didn't find anything
						d.ID = nil;
					else
						tex = GetItemIcon(d.ID);
						if (not tex) and XBarCore.ModData[mod].ActionList[a].Texture then
				...
				elseif v.Type == XBAR_TEMOTE then
					tex = v.Texture; -- self is defined externally by the Mod
					XBarCore.ModData[mod].ActionList[a].ToolType=XBAR_TTEXT;
				elseif v.Type == XBAR_TCHAT then
					tex = v.Texture; -- self is defined externally by the Mod
					XBarCore.ModData[mod].ActionList[a].ToolType=XBAR_TTEXT;
				...
				elseif v.Type == XBAR_TTRACKING then
					...
					d.ID,tex = XBarCore.GetTrackID(d.Data); -- Find and save ID and texture
...
function XBarCore.GetSpellID(spell,book,rank)
	local i = 0;
	local spellName, spellRank, spellType;

	repeat
		i = i + 1;
		spellName, spellRank = GetSpellBookItemName(i, book);
		if ((spellName == spell) and ((rank == nil) or ((type(rank)=="string") and (spellRank == rank)))) then
			spellType = GetSpellBookItemInfo(i,book)
			break;
		end
	until (spellName==nil);
	...
	elseif (spellType == "FUTURESPELL") then
		-- If this is not a spell we know, then ignore it.
		i=nil;
	end
...
function XBarCore.GetTrackID(spell)
	local i = 0;
	local trackName,tex,active;

	repeat
		i = i + 1;
		trackName,tex,active = GetTrackingInfo(i);
		if (trackName == spell) then
			break;
		end
	until (trackName==nil);
...
function XBarCore.OnMouseDown(self,button)
	local pfn=self:GetParent():GetName();
...
function XBarCore.OnMouseUp(self,button)
	local pfn=self:GetParent():GetName();
	if (button == "RightButton" and XBarCore.FrameDB[pfn] and XBarCore.FrameDB[pfn].IsMoving) then
		local bn=self:GetName();
		local m=XBarCore.FrameDB[bn].XBarToggle;
		if ((m==nil) or (m=="-")) then
			self:GetParent():StopMovingOrSizing();
			XBarCore.SaveLoc(self:GetParent());
			if self:GetObjectType()=="Button" then
...
function XBarCore.Orient(mod)
	...
	-- self function cannot be performed in combat.
			...
			if (button~=nil) then
				...
				if (XBarCore.FrameDB[mod.."Button"..i].XBarSpacer) then
					...
					if action and XBarCore.FrameDB[mod.."Button"..i].XBarWrap==XBAR_SWRAP and orientrow > 0 then
						-- self is a new spacer that we can wrap on
...
function XBarCore.RegisterAddon(mod,autocheck,skipimport)
	-- self function is used by plugins to let us know they are present and loaded.
	local n,v1,v2,i;
	local dbcache={ };
	...
	if autocheck==nil then
		-- Error message (self will happen for pre-3.0 bars if they try to load with 3.0 or later)
		print(mod..XBAR_MSG5);
		return;
	end
	...
	if (XBarCore.ModData[mod].enabled) then
		...
		if not XBarCore.ModData[mod].sortable then
			-- if the mod is not sortable, we have to overwrite self value to default every time.
...
function XBarCore.SetTooltip(self)
	local bn=self:GetName();
	local pfn=self:GetParent():GetName();
	local a,v,id,t,d,r;
	...
	elseif (t == XBAR_TSPELL) then
		if id~=nil then
			-- Do nothing if the spell does not exist (a pet died in combat and the body disappeared is the only time self should happen)
			GameTooltip_SetDefaultAnchor(GameTooltip, self);
			GameTooltip:SetSpellBookItem(id, v.Data[1].AuxData);
			GameTooltip:Show();
		end
			...
			elseif (XBarData[XBarCore.XBarPlayerName]==msg) then
				...
				for x,v in pairs(XBarData) do
					if ((x~="dbver") and (strsub(x,1,9)~="optionset") and (type(v)=="string")) then
						-- By process of elimination, self has to be a player's option set
						if (XBarData[x]==msg) then
							XBarData[x]="1"; -- Found one that was set to self option set
						end
					end
				end
					...
					for x,v in pairs(XBarData) do
						if ((x~="dbver") and (strsub(x,1,9)~="optionset") and (type(v)=="string")) then
							...
							if (XBarData[x]==tostring(n)) then
								XBarData[x]=msg; -- Found one that was set to self option set
							end
						end
					end
				end
...
function XBarCore.Start()
	...
	if (XBarInitialized) then
		return; -- Only do self if we haven't done it yet
	end
	...
	if (resetdb) then
		--Data structure not initialized for self player, so set one up
		XBarData={
			["dbver"]=dbver;
			["optionset-default"] = {};
			["noptionsets"] = 1;
...
function XBarCore.Toggle(self,button,down)
	if not IsControlKeyDown() then
		local mod=self:GetName();
...
function XBarCore.UpdateBar(mod,forceupdate)
	...
	if (XBarData[XBarCore.XBarOptionSet]) then
		...
		for n,key in pairs(XBarData[XBarCore.XBarOptionSet].mods[mod].sortorder) do
			...
			-- Detect corrupt sortlists
			if (not action) then
				--print("XBar: Corrupt sort index in "..mod.." at "..key);
				--skip it
			else
				...
				elseif (action.Type ~= nil) then
					...
					-- Notice we're only doing self for ITEM type buttons, not MACRO type buttons with items in them
					...
					if (id) then
						...
						-- If self or a previous button had to be updated, lets do all the others
						if (update and not InCombatLockdown()) then
							...
							if b1 then
								...
								-- Set owner to the bar itself, and priority override.
								b=SetOverrideBindingClick(getfenv()[mod],true,b1,mod.."Button"..i);
							else
								...
								for b=1,#(action.Data) do
									...
									if b1 then
										...
										if action.Data[b].IFF~="" then
											button:SetAttribute("*unit*","target");  -- Need to specify a unit for self to work.
...
function XBarCore.UpdateCooldowns(mod)
	...
	for i = 1, n do
		...
		if cd then
			-- Make sure we have a cooldown on self frame.
			start=0;
			duration=0;
			enable=1;
			if cd=="I" and type(v) == "number" then
				-- Can't handle items by name anymore :(
			...
			if (start ~= nil) and (duration ~= nil) and (enable ~= nil) and
			   (start > 0) and (duration > 0) then
				-- Save the start time so we don't trigger self more than once
				...
				if not XBarCore.FrameDB[mod.."Button"..i].StartCD then
					...
					if XBarCore.FrameDB[mod.."Button"..i].StartCD == start then
						-- Don't show self more than once.
						start=nil;
					else
						XBarCore.FrameDB[mod.."Button"..i].StartCD = start;
					end
...
function XBarLoader.OnEvent(self,event,...)
	local i,v,f;
...
function XBarLoader.BagUpdate()
	local i,v,f;
	-- We're updating in self frame to reduce calls to the inventory update function
	XBarCore.UpdateInventory();
...
function XBarLoader.OnLoad(self)
...
function XBarBindGUI.BtnBind_OnClick(self)
...
function XBarBindGUI.BtnBind_OnMouseDown(self,button)
...
function XBarBindGUI.BtnBind_OnKeyDown(self,key)
...
function XBarBindGUI.BtnClose_OnClick(self)
...
function XBarConfigGUI.BtnHelp_OnClick(self,context)
	if context==nil then
...
function XBarConfigGUI.CheckButtonH_OnClick(self)
	local mod=XBarConfigCurMod:GetText();
...
function XBarConfigGUI.CheckButtonL_OnClick(self)
	local mod=XBarConfigCurMod:GetText();
...
function XBarConfigGUI.CheckButtonO_OnClick(self)
	local mod=XBarConfigCurMod:GetText();
...
function XBarConfigGUI.CheckButtonTG_OnClick(self)
	local mod=XBarConfigCurMod:GetText();
...
function XBarConfigGUI.CheckButtonTT_OnClick(self)
	local mod=XBarConfigCurMod:GetText();
...
function XBarConfigGUI.CheckButtonV_OnClick(self)
	local mod=XBarConfigCurMod:GetText();
	local bar=getfenv()[mod];
...
function XBarConfigGUI.CheckButtonW_OnClick(self)
	local mod=XBarConfigCurMod:GetText();
...
function XBarConfigGUI.CheckMenu_OnClick(self)
...
function XBarConfigGUI.CheckMenu_Select(self)
	local mod=XBarConfigCurMod:GetText();
	local v;
	local c=XBarCore.ModData[mod]["check"..self.value];
...
function XBarConfigGUI.CheckMenuFrame_OnLoad(self,level)
...
function XBarConfigGUI.ModMenu_OnClick(self)
...
function XBarConfigGUI.ModMenu_Select(self)
	local mod=self.value;
	XBarConfigCurMod:SetText(mod);

	if (InCombatLockdown()) then
		return; -- can't do self in combat
	end
...
function XBarConfigGUI.ModMenuFrame_OnLoad(self,level)
	-- Loading off the click event from the button itself so we can make sure the data is there.
	-- Reinitializes every time we click the button.
	local info = UIDropDownMenu_CreateInfo();
	local n,i,mod,t,v;
...
function XBarConfigGUI.OnHide()
	if (XBarBind:IsVisible() and not InCombatLockdown()) then
		XBarBind:Hide();
	end
end
...
function XBarConfigGUI.ProfileBtnDel_OnClick(self)
...
function XBarConfigGUI.ProfileBtnNew_OnClick(self)
...
function XBarConfigGUI.ProfileBtnUse_OnClick(self)
	local p=XBarCore.FrameDB.XBarOptionsProfilesCurProfile;
...
function XBarConfigGUI.ProfileMenu_OnClick(self)
...
function XBarConfigGUI.ProfileMenu_Select(self)
	local p=tostring(self.value);
...
function XBarConfigGUI.ProfileMenuFrame_OnLoad(self,level)
	-- Loading off the click event from the button itself so we can make sure the data is there.
	-- Reinitializes every time we click the button.
	local info = UIDropDownMenu_CreateInfo();
	local t,i,v;
...
function XBarConfigGUI.SetBinding_OnClick(self)
...
function XBarConfigGUI.SliderScale_OnChange(self)
	local mod=XBarConfigCurMod:GetText();
...
function XBarConfigGUI.SliderMenu_OnClick(self)
...
function XBarConfigGUI.SliderMenu_Select(self)
	local mod=XBarConfigCurMod:GetText();
	local v=self.value;
	...
	if v2==nil then
		...
		v2=XBarCore.ModData[mod]["dslider"..v];
	end
...
function XBarConfigGUI.SliderMenuFrame_OnLoad(self,level)
	-- Loading off the click event from the button itself so we can make sure the data is there.
	-- Reinitializes every time we click the button.
	local info = UIDropDownMenu_CreateInfo();
	local n,i,mod;
...
function XBarConfigGUI.SliderMulti_OnChanged(self,value)
	...
	XBarConfigSliderMultiVal:SetText(format(fmt, value));
	XBarData[XBarCore.XBarOptionSet]["mods"][mod].Options[c]=value;
	local f=getfenv()[XBarCore.ModData[mod]["foptioncb"]];
...
function XBarConfigGUI.SortDown_OnClick(self)
...
function XBarConfigGUI.SortUp_OnClick(self)
	local v=XBarCore.FrameDB.XBarConfigScrollOrder.XBarCur;
	local mod=XBarConfigCurMod:GetText();
	local n=#(XBarData[XBarCore.XBarOptionSet].mods[mod].sortorder);
	local t;
...
function XBarHelpGUI.BtnClose_OnClick(self)
	if not InCombatLockdown() then
		XBarHelp:Hide();
	end
end
...
function XBarHelpGUI.OnLoad(self)
	local c,t;
...
function XBarScrollFrameTemplate_OnScroll(self)
	local fn = self:GetParent():GetName();
...
function XBarScrollFrameTemplate_OnClick(self,i)
	local fn = self:GetParent():GetName();
