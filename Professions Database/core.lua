...
addon.hooktradeskill = function(self)
  ...
  GetTradeSkillReagentInfo = function(i, j)
    local reagentName, reagentTexture, reagentCount, playerReagentCount = oGetTradeSkillReagentInfo(i, j)
    playerReagentCount = playerReagentCount + getReagentInfo(reagentName)
    return reagentName, reagentTexture, reagentCount, playerReagentCount
  end
  GetTradeSkillInfo = function(i, inc)
    local skillName, skillType, numAvailable, isExpanded, serviceType, numSkillUps = oGetTradeSkillInfo(i)
    if inc and skillType ~= "header" then
      ...
      numAvailable = tonumber(format("%d", num == 2^8 and 0 or num)) or numAvailable or 0
    end
    return skillName, skillType, numAvailable, isExpanded, serviceType, numSkillUps
  end
  ...
  -- [[ TODO: trying to reduce high load and performance instability in this segment :(
