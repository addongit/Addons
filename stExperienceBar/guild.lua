...
local barHeight, barWidth = stExp.guildHeight, stExp.guildWidth
local Anchor = stExp.guildAnchor
...
local function updateStatus()
	...
	if not IsInGuild() then
		...
		mouseFrame:SetScript("OnEnter", function()
			...
			if not stExp.GuildIsMaxLevel() then
				GameTooltip:AddLine(GetGuildInfo("player").."'s Experience:")
				GameTooltip:AddLine(string.format('Level: %s',UnitGetGuildLevel("player")))
				GameTooltip:AddLine(string.format('XP: %s/%s (%d%%)', stExp.ShortValue(XP), stExp.ShortValue(remXP+XP), (XP/(remXP+XP))*100))
				GameTooltip:AddLine(string.format('Remaining: %s', stExp.ShortValue(remXP)))
				GameTooltip:AddLine(string.format('Daily XP: %s/%s', stExp.ShortValue(dailyXP), stExp.ShortValue(dailymaxXP)))
				GameTooltip:AddLine(" ")
			end
			for i=1,GetNumFactions() do
				if select(1, GetGuildInfo("player")) == select(1, GetFactionInfo(i)) then
					local name, _, rank, minRep, maxRep, value = GetFactionInfo(i)
					GameTooltip:AddLine(string.format('%s Reputation:', name))
					GameTooltip:AddLine(string.format('Standing: |c'..stExp.Colorize(rank)..'%s|r', stExp.FactionInfo[rank][2]))
					GameTooltip:AddLine(string.format('Rep: %s/%s (%d%%)', stExp.CommaValue(value-minRep), stExp.CommaValue(maxRep-minRep), (value-minRep)/(maxRep-minRep)*100))
					GameTooltip:AddLine(string.format('Remaining: %s', stExp.CommaValue(maxRep-value)))
				end
			end
