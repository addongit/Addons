...
local HealBot_Options_SelectHealSpellsCombo_List = {
  ...
  --  HEALBOT_HOLY_WORD_CHASTISE,  -- Buggy in the current WoW 4.0.3
}
...
function HealBot_Options_idleInit()
    if not DoneInitTab[0] then
        if HealBot_Config.DebuffWatch==1 and HealBot_Globals.preloadOptions==1 then
            DoneInitTab[0]=400
        elseif HealBot_Config.BuffWatch==1 and HealBot_Globals.preloadOptions==1 then
            DoneInitTab[0]=500
        else
            DoneInitTab[0]=9999
        end
    elseif DoneInitTab[0]>0 then
        ...
        if DoneInitTab[0]>414 and DoneInitTab[0]<499 then
            if HealBot_Config.BuffWatch==1 and HealBot_Globals.preloadOptions==1 then
                DoneInitTab[0]=500
            else
                DoneInitTab[0]=9999
            end
        elseif DoneInitTab[0]>520 then
            DoneInitTab[0]=0
        else
...
function HealBot_Options_InitSub1(subNo)
    if subNo==401 and not DoneInitTab[401] then
...
function HealBot_Options_SetSkinBars()
    ...
    HealBot_BarIHCustomColourt:SetFont(LSM:Fetch('font',Healbot_Config_Skins.btextfont[Healbot_Config_Skins.Current_Skin]),Healbot_Config_Skins.btextheight[Healbot_Config_Skins.Current_Skin],"");
