 --Mage Nuggets 2.079 by B-Buck (Bbuck of Eredar)
...
function MageNuggets_SlashCommandHandler(msg) --Handles the slash commands
    ...
    DEFAULT_CHAT_FRAME:AddMessage("|cffffffff------------|cff00BFFF".."Mage".." |cff00FF00".."Nuggets".."|cffffffff 2.079--------------")
...
-------------------------------------
                            ...
                            DEFAULT_CHAT_FRAME:AddMessage("|cff00BFFF".."Ice Barrier Absorbing"..":|cffFFFFFF "..iceBarrRounded.."|cff00BFFF ".."Damage");
-------------------------------------                        
                        end                
                        if (arg == 1463) then
                            ...
                            DEFAULT_CHAT_FRAME:AddMessage("|cff00688B".."Mana Shield Absorbing"..":|cffFFFFFF "..manaShieldRounded.."|cff00688B ".."Damage");
                        end
                    end
                end    
            end
        ...
        if event1 == "SPELL_AURA_APPLIED" then
            ...
            if (MageNugz.polyToggle == true) then
                ...
                if (arg == 543) then
                    if(MageNugz.absorbToggle == true) and (MageNugz.consoleTextEnabled == true) then
                        local mageWardTotal = ((GetSpellBonusDamage(3) * 0.8067) + 1950);
                        local mageWardRounded = math.floor(mageWardTotal*math.pow(10,0)+0.5) / math.pow(10,0)              
                        DEFAULT_CHAT_FRAME:AddMessage("|cff00BFFF".."Mage Ward Absorbing"..":|cffFFFFFF "..mageWardRounded.."|cff00BFFF ".."Frost, Fire, or Arcane".." ".."Damage");
                    end         
...
function MNVariablesLoaded_OnEvent() --Takes care of the options on load up
        ...
        if(mnenglishClass == 'WARLOCK') then
            DEFAULT_CHAT_FRAME:AddMessage("|cff00BFFF".."Mage".."|cff00FF00".."Nuggets".."|cffffffff 2.079 ".."loaded! Some Options Disabled (Class:"..UnitClass("Player")..")")
            MageNugz.igniteTog = false;
            MageNugz.ssMonitorToggle = false;
            MageNugz.mageProcToggle = false;
            MageNugz.absorbToggle = false;
        ...
        if(mnenglishClass == 'MAGE') then
            DEFAULT_CHAT_FRAME:AddMessage("|cff00BFFF".."Mage".."|cff00FF00".."Nuggets".."|cffffffff 2.079 ".."loaded! (Use: /magenuggets options)")
            MageNugCD_Frame_Text:SetText(" ");
            MageNugz.moonkinTog = true;
            MageNugMoonkin_Frame:Hide()
            MNmoonFire_Frame:Hide();
        ...
        MNshieldFontString:SetText("Disable Precise Shield Absorb Notify")
...
function MNmoonFire_OnUpdate(self, elapsed)
    ...
    if (self.TimeSinceLastUpdate > 0.1) then   
        ...
        if(moonfireTime <= 0.5) then
            MNmoonFire_FrameText:SetText("cast")                
            if(MageNugz.castBoxes == true)then
                if(incombat == 1) then
...
function MNinsectSwarm_OnUpdate(self, elapsed) 
    ...
    if (self.TimeSinceLastUpdate > 0.1) then   
        ...
        if(insectTime <= 0.5) then
            MNinsectSwarm_FrameText:SetText("cast")                
            if(MageNugz.castBoxes == true)then
                if(incombat == 1) then
...
function MNstarSurge_OnUpdate(self, elapsed) 
    ...
    if (self.TimeSinceLastUpdate > 0.1) then   
        ...
        elseif (starsurgeTime > 1.0) then
            ...
            MNstarSurge_FrameText:SetText("cast")
            if(MageNugz.castBoxes == true)then
               if(incombat == 1)then
