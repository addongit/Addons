 --Mage Nuggets 2.094 by B-Buck (Bbuck of Eredar)
...
function MageNuggets_SlashCommandHandler(msg) --Handles the slash commands
    ...
    DEFAULT_CHAT_FRAME:AddMessage("|cffffffff------------|cff00BFFF".."Mage".." |cff00FF00".."Nuggets".."|cffffffff 2.094--------------")
...
function MageNuggets_OnEvent(this, event, ...)
    ...
    if (event == "COMBAT_LOG_EVENT_UNFILTERED")then   
        local _, event1, temp1, temp2, sourceName, _, destGUID, destName = select(1, ...) 
        local arg2, arg, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10, arg11, arg12 = select(9, ...) 
		local spellId1 = select(13, ...)     
        ...
        if(MageNugz.igniteTog == true)then
            if((event1 == "SPELL_DAMAGE") and (sourceName == UnitName("player")))then
                if(arg11 == 1)then
                    if (arg7 == 4) or (arg7 == 20) then
                        local ignitename, _, _, _, ignitecurrRank, _ = GetTalentInfo(2,4) --ignite 
                        if (ignitecurrRank > 0) then
                            if(destGUID == UnitGUID("target")) then  
                                local mastertemp = ((GetCombatRatingBonus(26)+8)*2.8)
                                igniteTemp = RoundZero((arg5*0.40)*(1+(0.01*mastertemp)));
                                ignitetimer = 5.0;   
...
function MNVariablesLoaded_OnEvent() --Takes care of the options on load up
        ...
        if(mnenglishClass == 'WARLOCK') then
            DEFAULT_CHAT_FRAME:AddMessage("|cff00BFFF".."Mage".."|cff00FF00".."Nuggets".."|cffffffff 2.094 ".."loaded! Some Options Disabled (Class:"..UnitClass("Player")..")")
            MageNugz.igniteTog = false;
            MageNugz.ssMonitorToggle = false;
            MageNugz.mageProcToggle = false;
            MageNugz.mirrorImageToggle = false;
        ...
        if(mnenglishClass == 'MAGE') then
            DEFAULT_CHAT_FRAME:AddMessage("|cff00BFFF".."Mage".."|cff00FF00".."Nuggets".."|cffffffff 2.094 ".."loaded! (Use: /magenuggets options)")
            MageNugCD_Frame_Text:SetText(" ");
            MageNugz.moonkinTog = true;
            MageNugMoonkin_Frame:Hide()
            MNmoonFire_Frame:Hide();
