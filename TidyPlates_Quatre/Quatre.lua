...
WidgetConfig.ThreatWidget = { anchor =  "TOP", x = 0 ,y = 20 }	-- y = 20
--WidgetConfig.ThreatWidget = { anchor =  "TOP", x = 0 ,y = -2 }	-- y = 20
...
local function OnApplyThemeCustomization(theme)
	...
	theme.frame.y = ((LocalVars.FrameVerticalPosition-.5)*50)-16

	--[[ Font
	local currentFont = font
	if LocalVars.TextUseBlizzardFont then currentFont = blizzfont end
	theme.name.typeface = currentFont
	theme.level.typeface = currentFont
	theme.customtext.typeface = currentFont
	theme.spelltext.typeface = currentFont
	--]]
