...
--Default values    
    ...
    if (combulbtarget==nil) then combulbtarget = true end -- set lb tracker complete mode upon very first launch 
...
combudefaultsettingstable = {  barcolornormal = {0,0.5,0.8,1},
...
function Combustionreset ()
		...
		combusettingstable = combudefaultsettingstable
...
function CombustionAutohide()
    ...
    local a5,b5,c5 = GetSpellCooldown(comb)
 	...
 	if (a5 == nil) then
	elseif combuautohide == 1 and combufaded == true then -- condition when autohide is disabled
		combufaded = false
        table.wipe(LibTransition.tQueue) table.wipe(LibTransition.cQueue) -- stop the Libtransition queue
		CombustionFrame:FadeIn(combufadeinspeed)
	elseif combucombat == nil and combufaded == false and ((combuautohide == 2) or (combuautohide == 3)) then -- condition when not in combat and CH is visible
		combufaded = true
		CombustionFrame:FadeOut(combufadeoutspeed)
	elseif combuautohide == 2 and combucombat == 1 and combufaded == true then -- condition when in combat
		combufaded = false
        table.wipe(LibTransition.tQueue) table.wipe(LibTransition.cQueue)
		CombustionFrame:FadeIn(combufadeinspeed)
	elseif combuautohide == 3 and combucombat == 1 and combufaded == true and ((a5 + b5 - GetTime()) <= (combuafterfade + combufadeinspeed)) then -- condition when in combat and CB is up
		combufaded = false
        table.wipe(LibTransition.tQueue) table.wipe(LibTransition.cQueue)
		CombustionFrame:FadeIn(combufadeinspeed)
	elseif combuautohide == 3 and combucombat == 1 and combufaded == false and ((a5 + b5 - GetTime()) >= (combuafterfade + combufadeinspeed)) and ((a5 + b5 - GetTime()) <= (120 - combubeforefade)) then -- condition when in combat and CB is in cd
		combufaded = true
		CombustionFrame:FadeOut(combufadeoutspeed,combufadealpha)
        CombustionFrame:Wait((a5 + b5 - GetTime())-combufadeinspeed-combubafterfade)
        CombustionFrame:FadeIn(combufadeinspeed)
		StatusTextFrameLabel:SetText("Autohiding now.")
        if (combuchat==true) then ChatFrame1:AddMessage(format("|cff00ffffCombustion Helper back in %d seconds|r", (a5 + b5 - GetTime())-combufadeinspeed-combubafterfade))
        end
    end
...
function CombustionIgnite(event, spellId, spellSchool, amount, critical, destGUID)
	...
	if ((critical == 1) and (event == "SPELL_DAMAGE")) and ((spellSchool == 4) or (spellSchool == 20)) and (spellId ~= 83853) and (spellId ~= 89091) and (spellId ~= 44461) and (spellId ~= 2120) and (spellId ~= 88148) and (spellId ~= 82739) and (spellId ~= 83619) then
		...
		if (combuignitepredict == true) then 
            combuigndamage = format("%.0f",combuignitebank / combuignitecount)
            IgniteLabel:SetText(format("|cffffffff*%.0f Dmg|r", combuigndamage))
        end
	...
	elseif (event == "SPELL_PERIODIC_DAMAGE") and (spellId == 12654) then
	    ...
	    if (combuignitetemp ~= 0) and (combuignitetimer ~= 0) then
   	    ...
   	    if (combuignitepredict == true) then -- color ignite text depending of accuracy of predicter
         	if (tonumber(abs(combuigndamage-amount)) <= 3) then
 				IgniteLabel:SetText(format("|cff00ff00%.0f Dmg|r", amount))
 			else IgniteLabel:SetText(format("|cffff0000%.0f Dmg|r", amount))
 			end
 		elseif (combureport == true) then 
 			IgniteLabel:SetText(format("|cffffffff%.0f Dmg|r", amount))
 		end
		
		combuigndamage = amount
	    combuignitetemp = 0
                            
	elseif ((event == "SPELL_AURA_REMOVED") and (spellId == 12654)) or (combucrittarget ~= UnitGUID("target")) or (UnitGUID("target") == nil) then
	...
	if (combubartimers == true) then
		if (k2==12654) and (combuignitetimer <= combutimervalue) then 
			Ignbar:Show()
...
-- FlameStrike tracking 
			    if  (combuflamestrike == true) or (spellId == 2120) and (spellId == 88148) then 
...
-- Start and End of fight events 
    ...
    elseif (event == "PLAYER_REGEN_ENABLED") then 
    	...
    	if (combureportmunching == true) and (combuignitevalue ~= 0) then 
            print("ignite expected :",combuignitevalue,"-- ignite applied :",combuigniteapplied,"-- ignite lost :",format("%.0f / %.0f%%",combuignitevalue-combuigniteapplied,(combuignitevalue-combuigniteapplied)/combuignitevalue*100)) 
        end
        ...
        if (combureportpyro == true) and (combupyrogain ~= 0) then
            print(format("|cffff0000 -- Hot Streak gained : |cffffffff%d  |cffff0000-- Hot Streak cast : |cffffffff%d  / %d%% |cffff0000--|r",combupyrogain + combupyrorefresh, combupyrocast, (100*(combupyrocast/(combupyrogain + combupyrorefresh)))))
        end
