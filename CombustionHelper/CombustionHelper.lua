...
local LBTime,FFBTime,IgnTime,PyroTime,CombustionUp,ffbglyph,combufadeout,impactup,ffbheight,critheight,combucritwidthl,lbraidcheck,lbtablerefresh,combuimpacttimer,combulbrefresh,lbraidcheck,lbtablerefresh,lbgroupsuffix,lbtargetsuffix,lbgroupnumber,lbtrackerheight,lbframeprefix,combupyrogain,combupyrorefresh,combupyrocast
    ...
    elseif lbgroupsuffix then
 	...
 	if (lbraidcheck == 0) -- info with UnitAura have been collected, skipping this part.
        then for z = 1, #LBtrackertable do -- no raid members targetting the LB's target so using GetTime from event fired and 12 secs as duration
             ...
             if (lbtablerefresh == 1) then
             else table.insert(LBtrackertable, {targetguid, targetname, (eventgettime + 12), 12})
             end
	end
end
...
-- Combustion on target tracking
    if (combutrack==true) then
			...
			if (k11==83853) and (combudottimer <= 2)
				then Combubar:Show()
 ...
 -- autohide part 
	...
	elseif ((a5 + b5 - time) <= (120 - combubeforefade)) 
	   ...
	   then combufadeout = true
	     	if (combuchat==true) then StatusTextFrameLabel:SetText(format("Autohiding now.")) end
									    CombustionFrame:FadeOut(combufadeoutspeed,combufadealpha);
									    CombustionFrame:Wait(combuwaitfade);
									    CombustionFrame:FadeIn(combufadeinspeed);
            if (combuchat==true) then ChatFrame1:AddMessage(format("|cff00ffffCombustion Helper back in %d seconds|r", 120-combubeforefade-combuafterfade-combufadeoutspeed-combufadeinspeed)) end
	end
