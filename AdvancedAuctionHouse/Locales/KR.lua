...
모든 기능들을 경매장 화면에 넣었으므로 더 이상 경매장 화면을 여는 것 이외에 다른 일을 할 필요가 없이 새로운 기능을 곧바로 사용할 수 있습니다.]=],
	...
	AUCTION_LOADED_MESSAGE = "Addon loaded: Advanced AuctionHouse <VERSION> by Mavoc (KR)",
...
오른쪽 클릭 : 행 변경]=],
	...
	GENERAL_GENERAL = "전체", -- Needs review
...
예: AVG - ((AVG - MIN) / 3)]=],
	...
	SETTINGS_FILTER_SPEED_TEXT = [=[이 설정은 각 업데이트 때마다 필터링하는 아이템 수량을 조절합니다. 높게 설정하면 빠르지만 필터링 시간이 오래 걸릴 수 있습니다.
마우스 휠 기능을 지원합니다.]=], -- Needs review
	SETTINGS_GENERAL = "일반 설정",
	SETTINGS_HISTORY = "기록 설정",
	SETTINGS_MAX_SAVED_HISTORY = "최대 기록 저장",
	SETTINGS_MAX_SAVED_HISTORY_HEADER = "최대 기록 저장",
...
체크 해제 :  ALT키를 누르고 있을 때만 툴팁 표시]=],
	SETTINGS_PRICE_PER_UNIT_PER_WHITE = "가격/낱개별/순수 원재료", -- Needs review
	SETTINGS_PRICE_PER_UNIT_PER_WHITE_HEADER = "가격/낱개별/원재료", -- Needs review
	SETTINGS_PRICE_PER_UNIT_PER_WHITE_TEXT = [=[이 기능을 사용하면 모든 재료를 순수 원재료의 낱개별 가격으로 할 수 있게 됩니다.
다른 등급의 재료 가격을 좀 더 쉽게 비교할 수 있게 됩니다.
판매 가격을 포함한 낱개별 가격을 사용할 수 있도록 합니다.

Green = 2x White Mats (녹색 = 2x 원재료)
Blue = 12x White Mats (파랑 = 12x 원재료)
Purple = 72x White Mats (보라 = 72x 원재료)

사용 예시 :
100x |cff0072bc[주석 모래]|r (120,000으로 하려면)
가격/낱개별 = 1,200
가격/낱개별/원재료 = 100]=], -- Needs review
	TOOLS_DAY_ABV = "일",
