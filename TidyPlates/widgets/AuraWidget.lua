...
--WatcherFrame:RegisterEvent("UNIT_SPELLCAST_SENT")
...
local function WatcherFrameHandler(self, event, ...)
	
	-- UNIT_AURA & UNIT_SPELLCAST_SUCCEEDED
	local unitid
	if event == "UNIT_AURA" then 												-- UNIT_AURA fires for aura changes on target
		unitid = ...
		--print("UNIT_AURA", unitid, UnitGUID(unitid) )
		if unitid == "target" then
			local guid = UnitGUID("target")
			local frame = WidgetList_GUID[guid]
			if frame then UpdateWidget(frame, frame.unit) end
			--TidyPlates:Update()		-- General Update for Target
			TidyPlates:RequestDelegateUpdate()		-- Delegate Update
			
		end
	elseif event == "UNIT_SPELLCAST_SUCCEEDED" then								-- UNIT_AURA doesn't fire for mouseovers, so we catch the spell cast instead
		unitid = ...
		--print("UNIT_SPELLCAST_SUCCEEDED", unitid, UnitGUID(unitid) )
		if unitid == "player" then
			...
			if guid then
				...
				--TidyPlates:Update()		-- General Update for Target
				TidyPlates:RequestDelegateUpdate()		-- Delegate Update
				--print("mouseover", UnitName("mouseover"), guid, frame)	
			end
		end
	end
