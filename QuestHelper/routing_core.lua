QuestHelper_File["routing_core.lua"] = "4.1.0.177b"
...
function QH_Route_Core_ClusterAdd(clust, clustid_used)
  ...
  if not PriorityCount[0] then 
	  table.insert(Priorities, 0) 
...
function QH_Route_Core_SetClusterPriority_Internal(clustid, new_pri, force)
  ...
  if not PriorityCount[new_pri] then 
	  table.insert(Priorities, new_pri) 
	  table.sort(Priorities) 
  end
